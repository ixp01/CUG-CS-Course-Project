        ...
        for row in range(8):
            for col in range(8):
                ...
                if (self.game.get_current_player() == self.human_player and 
                    ...
                    painter.drawEllipse(rect.x() + 15, rect.y() + 15, 
                ...
                if piece != 0:
                    ...
                    painter.setPen(QPen(QColor(0, 0, 0), 1))
                    # 绘制圆形棋子
                    painter.drawEllipse(rect.x() + 5, rect.y() + 5, 
...
class StatusBar(QFrame):
    ...
    def __init__(self, board, parent=None):
        ...
        self.setStyleSheet("background-color: #E8E8E8; padding: 5px;")
    ...
    def update_status(self):
        ...
        if self.board.game.is_game_over():
            ...
            # 恢复正常窗口标题
            if hasattr(self.board, 'parent') and self.board.parent():
                main_window = self.board.parent().parent()
                if hasattr(main_window, 'setWindowTitle'):
                    main_window.setWindowTitle("反转棋 - MiniAlphaGo")
            ...
            if hasattr(self.board, 'parent') and self.board.parent():
                main_window = self.board.parent().parent()
                if hasattr(main_window, 'setWindowTitle'):
                    main_window.setWindowTitle("反转棋 - MiniAlphaGo (AI正在思考中...)")
        else:
            # 恢复正常状态显示和窗口标题
            self.status_label.setStyleSheet("color: black; font-weight: bold;")
            current_player = self.board.game.get_current_player()
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("反转棋 - MiniAlphaGo")
        ...
        # 创建状态栏
        self.status_bar = StatusBar(self.board)
        main_layout.addWidget(self.status_bar)
        
        # 创建按钮面板
        button_panel = QFrame()
        button_panel.setFrameShape(QFrame.StyledPanel)
        button_panel.setStyleSheet("background-color: #E8E8E8; padding: 5px;")
        button_layout = QHBoxLayout(button_panel)
        
        # 创建新游戏按钮
        new_game_btn = QPushButton("新游戏")
        new_game_btn.setFont(QFont("Arial", 12))
        new_game_btn.setStyleSheet("padding: 5px 15px;")
        ...
        # 设置窗口大小
        self.setFixedSize(self.board.board_size + 20, self.board.board_size + 100)
