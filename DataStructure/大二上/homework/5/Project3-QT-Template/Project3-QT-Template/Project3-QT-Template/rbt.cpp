...
RBT::RBT(std::vector<std::string>* v1,std::vector<std::string>* v2){
    TNULL = new RBTNode();
    TNULL->isRed = false;// 哨兵节点为黑色
    root = TNULL;
...
void RBT::insert(const std::string& word, const std::string& translation){
    RBTNode* node = new RBTNode(word, translation, nullptr, TNULL, TNULL, true);
    RBTNode* y = nullptr;
    RBTNode* x = root;
...
void RBT::Find(std::string s, std::vector<std::string>* vForEnglish, std::vector<std::string>* vForChinese, std::vector<std::string>* vforpath){
    ...
    // 输出查找过程中关键字对比的次数
    //std::cout << "红黑树对比次数: " << comparisonCount << std::endl;
}
