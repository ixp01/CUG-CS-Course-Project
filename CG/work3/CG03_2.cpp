...
void Display(void)
{
	...
	glTranslatef(0.0f, 0.0f, -250.0f);	   //将图形沿z轴负向移动（开始时视点在z=0处）

	glColor3f(1.0f, 0.0f, 0.0f);
	glutSolidSphere(12.0f, 15, 15);      // 绘制红色的原子
	//glutSolidTeapot(12.0);	// 换成茶壶
	{// 绘制位图文本
		char c[] = "Sun";
		int n = strlen(c);
		glRasterPos3d(0.0, 12.0, 0.0);		// 指定文本位置
		for (int i=0; i<n; i++)		
			glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, c[i]);
	}

	glColor3f(0.0f, 1.0f, 0.0f);
	glPushMatrix();                  // 保存当前的模型视图矩阵（因左乘矩阵，要注意模型变换的代码顺序与作用顺序相反）
		...
		//glutSolidTeapot(6.0);       
		{
			char c[] = "Earth";
			int n = strlen(c);
			glRasterPos3d(0.0, 6.0, 0.0);		// 指定文本位置
			for (int i=0; i<n; i++)		
				glutBitmapCharacter(GLUT_BITMAP_HELVETICA_18, c[i]);
		}
