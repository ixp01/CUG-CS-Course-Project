...
void MainWindow::on_pushButton_output_clicked()//发送信息按钮的函数，设置串口传输数据的最大值
{
    ...
    //         QByteArray data = packData();  // 封装数据
    //         tcpSocket->write(data);
    //     }
    // } else if (currentRole == Role::Server) {
    //     // Server发送逻辑：通过串口发送到硬件
    //     sendViaSerialPort();
    // }
...
void MainWindow::on_pushButton_Client_clicked()
{
  ...
  // ui->line_character->setText("Client");
  // currentRole = Role::Client;
  // closeExistingConnections();  // 关闭现有连接
    ui->line_character->clear();
    ui->line_character->setText("Client");
    currentRole = Role::Client;
    ...
    connect(tcpSocket, &QTcpSocket::readyRead, this, &MainWindow::clientReadData);
    connect(tcpSocket, &QTcpSocket::errorOccurred, this, &MainWindow::clientError);

    // 启用客户端功能
    ui->pushButton_pack->setEnabled(true);
    ui->pushButton_output->setEnabled(true);
    ui->pushButton_clearoutput->setEnabled(true);
    ui->pushButton_clearinput->setEnabled(true);
    ui->pushButton_clearack->setEnabled(true);
    ui->pushButton_stopoutput->setEnabled(true);
    ui->pushButton_clearwin->setEnabled(true);
    ui->pushButton_wrong->setEnabled(true);
    ui->pushButton_clearparity->setEnabled(true);
...
{
    ui->line_character->clear();
    ui->line_character->setText("Server");
    ...
    tcpServer->listen(QHostAddress::Any, 12345);  // 监听端口
...
void MainWindow::closeExistingConnections()
{
    // 关闭串口
...
{
  QMessageBox::about(NULL, "提示", "客户端已连接到服务器！");
}
...
void MainWindow::clientError(QAbstractSocket::SocketError socketError)
{
  QMessageBox::about(NULL, "错误", "客户端连接失败：" + tcpSocket->errorString());
}

void MainWindow::handleNewConnection()
{
    tcpSocket = tcpServer->nextPendingConnection();
    connect(tcpSocket, &QTcpSocket::readyRead, this, &MainWindow::serverReadData);
    connect(tcpSocket, &QTcpSocket::disconnected, this, &MainWindow::serverDisconnected);
    QMessageBox::about(NULL, "提示", "服务器已接受客户端连接！");
}
...
void MainWindow::serverDisconnected()
{
    tcpSocket->deleteLater();
    tcpSocket = nullptr;
    QMessageBox::about(NULL, "提示", "客户端已断开连接！");
}

void MainWindow::processNetworkData(QByteArray data)
{
    // 处理接收到的数据包
    QString in = QString(data);
    QStringList packets = in.split('\n');
    for (const QString &packet : packets)
    {
        if (packet.isEmpty()) continue;
